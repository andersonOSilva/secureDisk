...
class EmergencyInsuredModel(db.Model):
    ...
    policy_id = db.Column(db.Integer, db.ForeignKey('policy.id', ondelete='CASCADE', onupdate='CASCADE'))
    ...
    def get_by_policy(policy_id):
        return db.session.query(EmergencyInsuredModel).filter_by(policy_id=policy_id).all()
